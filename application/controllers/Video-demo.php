<?php
   ...
   class Video extends CI_Controller
   {
                ...
                for($i=0;$i<$count;$i++){
                  if(!empty($_FILES['files']['name'][$i])){
                    ...
                    if ($i==1) {
                         ...
                         {
                           $vdo.=$_FILES['files']['name'][$i];
                         }                                   
                    }                    
                    
                  }
                }        
                echo $result=$this->Video_Model->data_save($img,$vdo);
       }   
      ...
      public function dataUpdate()
       {
              if ($this->input->post('img') != "" || $this->input->post('thumnail')) {
                  $count = count($_FILES['files']['name']);
                  $img='';
                  $vdo='';
                  for($i=0;$i<$count;$i++){
                  ...
                  echo $result=$this->Video_Model->data_update($img,$vdo);      
              }
             else
             {
