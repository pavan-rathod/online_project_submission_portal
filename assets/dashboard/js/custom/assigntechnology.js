$(document).ready(function () {
    ...
    $("#save_modal").on('click', function () {
        $("#title").html("Assign Technology Form");
        $('.form-control').val('');
...
/*        var address = $('#txt_address').val();
 ...
 /*                   if (address != "") {    
                        if (password != "") { */                 
                              if (action == "Save") {
                                   $('#loadingDiv').show();
                                   $.post(base_url+'AssignTechnology/dataSave',{teacher_id: teacher_id,course:course,technology:technology/*,address:address,password:password*/},function(res){
                                        if (res>0)
                                         {
                                              loadData();                                                                                  
                                              swal({
                                                  title: "Well Done..!",
                                                  text: "Technology Assigned successfully.....!",
                                                  type: "success",
                                                  timer: 1000
                                              });
                                              $('.form-control').val('');
                                         ...
                                         { 
                                             swal("Sorry..!", "Unable to Assign Technology.........!", "error");
                                         }
                                  });                            
                              }
                              if (action == "Update") {
                                  var id = $('#txt_id').val();
                                  $('#loadingDiv').show();  
                                     $.post(base_url+'AssignTechnology/dataUpdate',{id:id,teacher_id: teacher_id,course:course,technology:technology/*,email:email,address:address,password:password*/},function(res){
                                          if (res>0)
                                           {
                                                loadData();                                                                                  
                                                swal({
                                           ...
                                           { 
                                               swal("Sorry..!", "Unable to Update Assignment .........!", "error");
                                           }
                                    });                       
                              }
/*                          } else {
                    ...
                    $('#btn_save').prop('disabled', false);
                    $('#save-result').html("<br>Please Select Name .....!");
                    setTimeout(function () { $('#save-result').html(' '); }, 3000);
                    $('#btn_save').prop('disabled', false);
                    $('#txt_name').focus();                
              }           
...
/* Load All form Data */
   function loadData()
    {
        if ($.fn.DataTable.isDataTable('#dataTable-Banner'))
         ...
         $.post(base_url+'AssignTechnology/loadData',{},function(res){
             var data =jQuery.parseJSON(res);
             //console.log(data);
             var html = "";              
             for (var i = 0; i < data['data']['length']; i++)
 ...
 /*                
    ...
    function getData(ele) {
        ...
        $.ajax({
            type: 'POST',
            url: base_url+'AssignTechnology/getRecord',
            data: {id:id},
            success: function (res) {
                var data = jQuery.parseJSON(res);
                console.log(data);
    ...
    function deleteData(ele) {
        ...
        swal({
            ...
            if (result == true) {
                $.ajax({
                    type: 'POST',
                     url: base_url+'AssignTechnology/dataDelete',
                    data: {id:id},
                    success: function (res) {
                        if (res == 1) {                            
                            loadData();
                            ...
                            swal("Sorry..!", "Unable to Delete Assignment .........!", "error");
                        }
                    }
                });
            }
